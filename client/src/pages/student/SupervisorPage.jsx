...
const SupervisorPage = () => {
  ...
  const submitRequest = () => {
    ...
    dispatch(requestSupervisor({
      teacherId: selectedSupervisor._id, message
    })).then(res => {
  ...
  return <>
    <div className="space-y-6">
          ...
          hasSupervisor ? (
            <div className="space-y-6">
              <div className="flex items-start space-x-6">
                <img className="w-20 h-20 rounded-full object-cover shadow-md" src="/placeholder.jpg" alt="Supervisor Avatar" />
                <div className="flex-1 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4" >
                    ...
                    <div>
                      ...
                      <p className=" font-medium text-slate-800">{Array.isArray(supervisor?.expertise) ? supervisor.expertise.join(", ") : (supervisor?.expertise || "-")}</p>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full font-medium capitalize text-sm ${project.status === "approved" ? "bg-green-100  text-green-800" : project.status === "pending" ? "bg-yellow-100  text-yellow-800" : project.status === "rejected" ? "bg-red-100  text-red-800" : "bg-gray-800"}`}>{project?.status || "Invalid"}</span>
                  </div>
                </div>
              </div>
      ...
      {/*  AVAILABLE SUPERVISOR | ONLY WHEN PROJECT EXIST AND NO SUPERVISOR ASSIGNED */}

      {hasProject && !hasSupervisor && (
        <div className="card">
          <div className="card-header">
            <h2 className="card-title" >Availabe Supervisors</h2>
            <p className="card-subtitle">Browse and request supervisor from availabe faculty members.</p>
          </div>
              ...
              supervisors && supervisors.map(sup =>  (
                 <div key={sup._id} className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-center space-x-3 mb-3">
                    <div className="flex w-12 h-12 bg-slate-300 rounded-full items-center justify-center">
                  ...
                  <div className="space-y-2 mb-4">
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-700">{sup.email || "-"}</p>
                    </div>
                  ...
                  <button onClick={() => handleOpenRequest(sup)} className="btn btn-primary w-full">Request Supervisor</button>
                </div>
              ))
            }
          </div>
          ...
          <div className="modal-content">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg">Request Supervision</h3>
                <button className="text-slate-400 hover:text-slate-600" onClick={() => { setShowRequestModal(false); setSelectedSupervisor(null); setRequestMessage(""); }}><X className="w-5 h-5" /></button>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea className="input min-h-[120px]" required value={requestMessage} onChange={(e) => setRequestMessage(e.target.value)} placeholder="Introduce yourself and explain why you'd like this professor to supervise your project..."/>
                </div>
                <div className="flex justify-end space-x-3 pt-4 border-t border-slate-200">
                  <button onClick={() => {setShowRequestModal(false); setSelectedSupervisor(null); setRequestMessage("")}} className="btn-outline">Cancle</button>
                  <button onClick={() => {submitRequest}} className="btn-outline" disabled={!requestMessage.trim()}>Send Request</button>
                </div>
              </div>
            </div>
          </div>
