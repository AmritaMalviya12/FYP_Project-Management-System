...
const AdminDashboard = () => {
  ...
  const [isReportModalOpen, setIsReportsModalOpen] = useState(false);
  ...
  const latestNotifications = useMemo(() => (notifications || []).slice(0, 6),
    [notifications]
  )
  ...
  return <>
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg p-6 text-white">
        ...
        <p className="text-blue-100">Manage the entire project managment system and oversee all activities.</p>
      </div>
            ...
            {supervisorBucket.length === 0 ? (
              ...
              <div className="h-72">
                <ResponsiveContainer width={"100%"} height={"100%"}>
                  <BarChart data={supervisorBucket}
                    ...
                    <Tooltip
                      ...
                      labelFormatter={(label) => `Supervsor: ${label}`}
                    />
                    <Bar dataKey="count" radius={[8, 8, 0, 0]}>
                      {supervisorBucket.map((entry, index) => {
                        const colors = ["#1E3A8A", "#2563EB", "#3B82FA", "#60A5FA", "#93C5FD"];
                        return (
                          <Cell key={`cell-${index}`}
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            ...
            {latestNotifications.length === 0 && (
              <div className="text-slate-500 text-sm ">No recent notification</div>
            )
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl mx-4 max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button className="text-slate-400 hover:text-slate-600" onClick={() => setIsReportsModalOpen(false)}><X className="w-5 h-5" /></button>
            </div>

            <div className="mb-4">
              <input type="text"
                className="input w-full"
                placeholder="Search by file naem, project title, or student name"
                value={reportSearch}
