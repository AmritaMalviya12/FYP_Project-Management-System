...
export const getStudentProject = asyncHandler(async (req, res, next) => {
    ...
    if (!project) {
        return res.status(200).json({
            success: true,
            data: { project: null },
            message: "No project found fot this student."
        });
    }
    res.status(200).json({
        success: true,
...
export const submitProposal = asyncHandler(async (req, res, next) => {
    ...
    if (existingProject && existingProject.status !== "rejected") {
        return next(new ErrorHandler("You already have an active project. You can only submit a new proposal if the previous project one was rejected. ", 400))
    }
    if(existingProject && existingProject.status === "rejected"){
...
export const getAvailableSupervisors = asyncHandler(async (req, res, next) => {
    const supervisors = await User.find({ role: "Teacher" }).select("name email department expertise").lean();// to read data lean()
    res.status(200).json({
        success: true,
        data: { supervisors },
...
export const getSupervisor = asyncHandler(async (req, res, next) => {
    ...
    if (!student.supervisor) {
        return res.status(200).json({
            ...
            message: "No supervisor asssigned yet",
        });
...
export const requestSupervisor = asyncHandler(async (req, res, next) => {
    ...
    res.status(201).json({
        success: true,
        data: { request },
        meassage: "Supervisor request submitted successfully.",
...
export const getDashboardStats = asyncHandler(async (req, res, next) => {
    ...
    re.status(200).json({
        ...
        data: {
            project, upcomingDeadlines, topNotifications, feedbackNotifications, supervisorName
        }
    })
